...
私有协作仓库：lqhoyh121-afk/xiehe-runtime-foundation-design。

先读[项目总控](PROJECT_CONTROL.md)、[协作规则](AGENTS.md)和[开工说明](docs/collaboration/START.md)。GitHub Issues负责执行状态，PR记录改动和审阅；总负责人批准最终合并/发布。
...
- `python -m pytest spikes/001-input-contract -q --import-mode=importlib` 不作为推荐入口（两片段有同名模块）。
