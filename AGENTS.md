...
采用经总负责人确认的单账号协作：GitHub必需Review批准数量为0，仍要求PR和CI；独立Agent审查只提供意见。唯一批准人为总负责人，合并与发布必须分别得到其针对具体PR/head的明确批准并记录到PR；批准调整规则不等于批准合并，head变化须重新确认。不启用auto-merge。同账号下GitHub不能强制区分用户和Agent，唯一批准权由流程落实，不夸称平台身份隔离。开发/测试不默认取得生产权限。
